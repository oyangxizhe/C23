...
namespace C23.SellManage
{
    public partial class FrmOrder : Form
    {
        ...
        protected string M_str_sql = @"select A.ORID as 订单号, A.SN as 项次,A.WareID as 品号,B.WNAME AS 品名,B.ExternalM as 套件,
           ...
           + "A.OCOUNT*A.SELLUNITPRICE*A.DISCOUNTRATE AS 未税金额,A.OCOUNT*A.SELLUNITPRICE*A.DISCOUNTRATE*A.TAXRATE/100 AS 税额,"
           + "A.OCOUNT*A.SELLUNITPRICE*A.DISCOUNTRATE*(1+A.TaxRATE/100) AS 含税金额, A.CuID as 客户代码,C.CName as 客户名称,C.PHONE AS 联系方式,"
           +" C.ADDRESS AS 送货地址,A.URGENT AS 加急否,A.ORDERDATE AS 订货日期,A.DELIVERYDATE AS 交货日期,A.Maker as 制单人,"
        ...
        string sql1 = "select distinct(ORID) from tb_ORDER";
        ...
        private void dgvStateControl()
        {
            ...
            for (i = 0; i < numCols1; i++)
            {
                ...
                else if (i == 0 || i == 2 || i == 11 || i == 12  || i==15 || i == 17 || i==16 || i==18 || i==20 || i==25)
                {
                    dataGridView1.Columns[i].Width = 90;

                }
                else if (i == 3 || i==19 || i==21)
                {
        ...
        private void ak()
        {
            ...
            if (cmb1.Enabled == true && dtpStartDate.Enabled == false)
            {
                ...
                {
                    dt = boperate.getdt(M_str_sql + " WHERE A.ORID LIKE '%" +cmb1 .Text  + "%' ORDER BY A.ORID,A.DATE ASC");
                    if (dt.Rows.Count > 0)
                    {
            ...
            else if (cmb1.Enabled == false && dtpStartDate.Enabled == true)
            {
                ...
                {
                    dt = boperate.getdt(M_str_sql + " WHERE A.DATE BETWEEN '" + v1 + "' AND '" + v2 + "' ORDER BY A.ORID ,A.DATE ASC");
            ...
            else if (cmb1.Enabled == true && dtpStartDate.Enabled == true)
            {
                ...
                if (cmb1.Text == "")
                {
                    MessageBox.Show("订单号不能为空！", "", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                ...
                else
                {
                    dt = boperate.getdt(@M_str_sql + " WHERE A.DATE BETWEEN '" + v1 + "' AND '" + v2 +
                        "' AND  A.ORID LIKE '%" + cmb1.Text + "%' ORDER BY A.ORID ,A.DATE ASC");
                    if (dt.Rows.Count > 0)
                    {
        ...
        private void dataGridView1_DoubleClick(object sender, EventArgs e)
        {
            if (getdata !=0)
            {
                if (getdata == 1)
                {
                    ...
                    C23.SellManage.FrmSellTableT.data4[0] = "doubleclick";
                    C23.SellManage.FrmSellTableT.data1[0] = s1;
                    C23.SellManage.FrmSellTableT.data1[1] = s2;
                    C23.SellManage.FrmSellTableT.data1[2] = s3;
                    C23.SellManage.FrmSellTableT.data1[3] = s4;
                    C23.SellManage.FrmSellTableT.data1[4] = s5;
            ...
            {
                FrmOrderT frm = new FrmOrderT();
                FrmOrderT.str6[0] = Convert.ToString(dataGridView1[0, dataGridView1.CurrentCell.RowIndex].Value).Trim();
                frm.ShowDialog();
            }
        }
        #endregion
        ...
        private void btnToExcel_Click(object sender, EventArgs e)
        {
            if (dt.Rows.Count > 0)
            {
                boperate.dgvtoExcel(dataGridView1, "订单明细");
            }
            else
            {
