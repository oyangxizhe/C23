...
{
    public partial class FrmPWGodE: Form
    {
...
" TB_WORKGROUP D ON D.WGID=A.WGID  LEFT JOIN TB_CRAFT E ON E.CRID=A.CRID LEFT JOIN TB_Storageinfo F ON F.STORAGEID=A.STORAGEID";
        ...
        string sql1 = "select distinct(godeid) from tb_gode WHERE SUBSTRING(GODEID,1,2)='PW' ";
        string sql4 = "select distinct(B.WNAME)  AS WNAME from tb_GODE A LEFT JOIN TB_WAREINFO B ON A.WAREID=B.WAREID WHERE SUBSTRING(A.GODEID,1,2)='PW'";
        public FrmPWGodE()
        {
            InitializeComponent();
        }
        ...
        {
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(FrmPWGodE));
            ...
            this.cmb2.Location = new System.Drawing.Point(344, 22);
            this.cmb2.Name = "cmb2";
        ...
        private void dgvStateControl()
        {
            ...
            for (i = 0; i < numCols1; i++)
            {
               ...
               else  if (i == 2 || i==3 || i==15 || i==16 ||  i==18)
                {
        ...
        private void tsbtnDel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    boperate.getcom("delete from tb_FrmGODEwhere WorkGroupM='" + Convert.ToString(dataGridView1[1, dataGridView1.CurrentCell.RowIndex].Value).Trim() + "'");
                    Bind();
                }
            }
            catch (Exception ex)
       ...
       private  void ak()
        ...
        {
            FrmPWGodET frm = new FrmPWGodET();
            FrmPWGodET.str6[0] = Convert.ToString(dataGridView1[2, dataGridView1.CurrentCell.RowIndex].Value).Trim();
            FrmPWGodET.str6[1] = Convert.ToString(dataGridView1[12, dataGridView1.CurrentCell.RowIndex].Value).Trim();
            frm.ShowDialog();
        }
        ...
        private void btnToExcel_Click(object sender, EventArgs e)
        {
            if (dt.Rows.Count > 0)
            {
                boperate.dgvtoExcel(dataGridView1, "计件工资入库明细");
            }
            else
            {
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (checkBox1.Checked)
                {
                    sql = @" WHERE A.GODEID LIKE '%" + cmb1.Text + "%' AND  SUBSTRING(A.GODEID,1,2)='PW'  AND A.DATE BETWEEN '" + v1 + "' AND '" + v2 +
              "' AND B.WNAME LIKE '%" + cmb2.Text + "%'  ORDER BY A.GODEID,A.GODEDATE ASC";

                }
                else
                {
                    sql = @" WHERE A.GODEID LIKE '%" + cmb1.Text + "%' AND  SUBSTRING(A.GODEID,1,2)='PW' AND B.WNAME LIKE '%" + cmb2.Text +
                        "%' ORDER BY A.GODEID,A.GODEDATE ASC";
