...
{
    public partial class FrmMiscGodET : Form
    {
...
+ "B.DETAIL AS 细节,B.Leather AS 皮种,B.COLOR AS 颜色,"
            + "B.StitchingC AS 线色,B.Thickness AS 海棉厚度,A.GECount as 入库数量,F.StorageType as 仓库,A.Goder as 入库员,A.GodEDate as 入库日期,"
+ " A.Maker as 制单人," +
"A.Date as 制单日期 from tb_GodE A LEFT JOIN TB_WAREINFO B ON A.WAREID=B.WAREID LEFT JOIN " +
" TB_WORKGROUP D ON D.WGID=A.WGID  LEFT JOIN TB_Storageinfo F ON F.STORAGEID=A.STORAGEID";
        ...
        private void FrmPurchaseInventoryT_Load(object sender, EventArgs e)
        {
        ...
        private void dgvStateControl()
        {
            ...
            for (i = 0; i < numCols1; i++)
            {
                ...
                else if (i == 2 || i == 11 || i == 13 || i == 14 || i == 15 || i == 16)
                {
                ...
                for (i = 0; i < dt.Rows.Count; i++)
                {
                    if (dt.Rows[i]["品号"].ToString() != "")
                    {
                        ...
                        string varsendValues = "('" + GEKEY + "','" + varSN + "','" + varID + "','" + year + "','" + month +
                         "','" + day + "','" + varWareID + "','" + varGECount + "','" + v7 + "','" + varGodEDate +
                      "','" + varGoder + "','" + varMaker + "','" + varDate + "')";
                        boperate.getcom("insert into tb_GodE(GEKEY,SN,GodEID,Year,Month,Day,WareID,"
                            + "GECount,StorageID,GodEDate,Goder,Maker,Date) values " + varsendValues);
                        #endregion
        ...
        private int ac(DataTable dt)
        {
            ...
            string v3 = dt.Rows[i][11].ToString();
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (dataGridView1.CurrentCell.ColumnIndex == 12)
            {
                ...
                {
                    dataGridView1.Rows[dataGridView1.CurrentCell.RowIndex].Cells[12].Value = inputTextDataStorage[0];
                    str5[0] = "";
