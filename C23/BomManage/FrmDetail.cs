...
namespace C23.BomManage
{
    public partial class FrmDetail : Form
    {
        ...
        protected string M_str_sql = "select Detail as 内部细节,Maker as 制单人,Date as 日期 from tb_Detail";
        protected string M_str_table = "tb_Detail";
        ...
        private void InitializeComponent()
        {
            ...
            this.groupBox1.Text = "内部细节信息";
            // 
            // txtDetail
            // 
            this.txtDetail.Location = new System.Drawing.Point(105, 22);
            this.txtDetail.Name = "txtDetail";
            this.txtDetail.Size = new System.Drawing.Size(192, 21);
        ...
        {
            txtDetail.Text = Convert.ToString(dataGridView1[0, dataGridView1.CurrentCell.RowIndex].Value).Trim();
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (txtDetail.Text == "")
                {
                    MessageBox.Show("内部细节不能为空！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else
                {
                    if (M_int_judge == 0)
                    {
                        dt1 = boperate.getdt("select Detail from tb_Detail where Detail='" + txtDetail.Text + "'");
                        if (dt1.Rows.Count > 0)
                        {
                            MessageBox.Show("内部细节已经存在！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        ...
                        {
                            boperate.getcom("insert into tb_Detail(Detail,Maker,Date) values('"
                                               + txtDetail.Text.Trim() + "','" + FrmLogin.M_str_name + "','" + varDate + "')");
                            Bind();
                    ...
                    {
                        dt2 = boperate.getdt("select Detail from tb_Detail");
                        if (dt2.Rows.Count > 0)
                        {
                            boperate.getcom(@"update tb_Detail set Detail='" + txtDetail.Text + "',Maker='" + FrmLogin.M_str_name +
                             "',Date='" + varDate +
                             "' where Detail='" + Convert.ToString(dataGridView1[0, dataGridView1.CurrentCell.RowIndex].Value).Trim() + "'");
                            Bind();
                        }
                        else
                        {
        ...
        private void btnDel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    boperate.getcom("delete from tb_Detail where Detail='" + Convert.ToString(dataGridView1[0, dataGridView1.CurrentCell.RowIndex].Value).Trim() + "'");
                    Bind();
                }
            }
            catch (Exception ex)
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    DataSet myds = boperate.getds(M_str_sql + " where Detail like '%" + txtKeyWord.Text.Trim() + "%' order by Date asc", M_str_table);
