...
namespace C23.BomManage
{
    public partial class FrmWorker : Form
    {
        ...
        protected string M_str_sql = "select WKID AS 工种代码,Worker as 工种,Maker as 制单人,Date as 日期 from tb_Worker";
        protected string M_str_table = "tb_Worker";
        ...
        private void InitializeComponent()
        {
            ...
            // txtWorker
            // 
            this.txtWorker.Location = new System.Drawing.Point(293, 20);
            this.txtWorker.Name = "txtWorker";
            this.txtWorker.Size = new System.Drawing.Size(192, 21);
            ...
            // txtWorkerID
            // 
            this.txtWorkerID.Location = new System.Drawing.Point(112, 20);
        ...
        private void Bind()
        {
            ...
            dt = boperate.getdt(M_str_sql + "  order by WKID,date asc");
        ...
        private void save()
        {
            ...
            else
            {
                if (M_int_judge == 0)
                {
                    dt1 = boperate.getdt("select Worker from tb_Worker where Worker='" + txtWorker.Text + "'");
                    if (dt1.Rows.Count > 0)
                    {
                        MessageBox.Show("工种已经存在！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);



                    }
                    else if (txtWorkerID.Text == "")
                    {

                        MessageBox.Show("工种代码不能为空！", "", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                    else
                    {
                        boperate.getcom("insert into tb_Worker(WKID,Worker,Maker,Date,Year,Month,Day) values('" + txtWorkerID.Text +
                            "','" + txtWorker.Text.Trim() + "','" + FrmLogin.M_str_name + "','" + varDate +
                            "','" + year + "','" + month + "','" + day + "')");
                        Bind();
                ...
                {
                    dt2 = boperate.getdt("select Worker from tb_Worker");
                    if (dt2.Rows.Count > 0)
                    {
                        boperate.getcom(@"update tb_Worker set Worker='" + txtWorker.Text + "',Maker='" + FrmLogin.M_str_name +
                         "',Date='" + varDate +
                         "' where Worker='" + Convert.ToString(dataGridView1[1, dataGridView1.CurrentCell.RowIndex].Value).Trim() + "'");
                        Bind();
                    }
                    else
                    {
        ...
        {
            txtWorkerID.Text = Convert.ToString(dataGridView1[0, dataGridView1.CurrentCell.RowIndex].Value).Trim();
            txtWorker.Text = Convert.ToString(dataGridView1[1, dataGridView1.CurrentCell.RowIndex].Value).Trim();
        }
        ...
        {
            string a1 = boperate.numN(12, 4, "0001", "select * from tb_Worker", "WKID", "WK");
        ...
        private void btnDel_Click(object sender, EventArgs e)
        {
            try
            {
                if (MessageBox.Show("确定要删除该条品号信息吗？", "提示", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
                {
                    ...
                    DataTable dty = boperate.getdt("select * from tb_workgroupM where wkid='" + v1 + "'");
                    if (dty.Rows.Count > 0)
                    {
                        MessageBox.Show("该工种已经在工作组中使用了，不允许删除，若要删除先删除工作组中该工种！", "", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                    else
                    {
                        boperate.getcom("delete from tb_Worker where Worker='" + v2 + "'");
                        Bind();
                    }
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    DataSet myds = boperate.getds(M_str_sql + " where Worker like '%" + txtKeyWord.Text.Trim() + "%' order by WKID,Date asc", M_str_table);
