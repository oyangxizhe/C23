...
namespace C23.WorkOrderManage
{
    public partial class FrmWorkOrder : Form
    {
        ...
        protected string M_str_sql = @"select distinct(A.WOID) as 工单号,A.WareID as 品号,B.WName as 品名,B.ExternalM as 套件,B.Type as 型号,
B.DETAIL AS 细节,B.LEATHER AS 皮种,COLOR AS 颜色,STITCHINGC AS 线色,THICKNESS AS 海棉厚度,C.VOUCHERPROPERTIES AS 单据性质,A.WORKORDERCOUNT AS 
工单数量,A.DATE AS 制单日期 FROM TB_WORKORDER A 
...
LEFT JOIN TB_VOUCHERPROPERTIES C ON C.VPID=A.VPID";
        ...
        string sql1 = "select distinct(woid) from tb_workorder";
        ...
        private void InitializeComponent()
        {
            ...
            this.chk1.Location = new System.Drawing.Point(57, 28);
            this.chk1.Name = "chk1";
            ...
            this.dtpEndDate.Location = new System.Drawing.Point(314, 50);
        ...
        private void tsbtnDel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    boperate.getcom("delete from tb_FrmSubContractwhere WorkGroupM='" + Convert.ToString(dataGridView1[1, dataGridView1.CurrentCell.RowIndex].Value).Trim() + "'");
                    Bind();
                }
            }
            catch (Exception ex)
        ...
        private void ak()
        {
            ...
            if (cmb1.Enabled == true && dtpStartDate.Enabled == false)
            {
                ...
                {
                    dt = boperate.getdt(M_str_sql + " WHERE A.WOID LIKE '%" +cmb1 .Text  + "%' ORDER BY A.WOID,A.DATE ASC");
                    if (dt.Rows.Count > 0)
                    {
            ...
            else if (cmb1.Enabled == false && dtpStartDate.Enabled == true)
            {
                ...
                {
                    dt = boperate.getdt(M_str_sql + " WHERE A.DATE BETWEEN '" + v1 + "' AND '" + v2 + "' ORDER BY A.WOID ,A.DATE ASC");
            ...
            else if (cmb1.Enabled == true && dtpStartDate.Enabled == true)
            {
                ...
                else
                {
                    dt = boperate.getdt(@M_str_sql + " WHERE A.DATE BETWEEN '" + v1 + "' AND '" + v2 +
                        "' AND  A.WOID LIKE '%" + cmb1.Text + "%' ORDER BY A.WOID ,A.DATE ASC");
                    if (dt.Rows.Count > 0)
                    {
        ...
        private void dataGridView1_DoubleClick(object sender, EventArgs e)
        {
            if (getdata !=0)
            {
                if (getdata == 1)
                {
                    ...
                    string s4 = this.dataGridView1.Rows[intCurrentRowNumber].Cells[11].Value.ToString().Trim();
                    C23.StorageManage.FrmOutSourcingMateReT.data4[0] = "doubleclick";
                    C23.StorageManage.FrmOutSourcingMateReT.data1[0] = s1;
                    C23.StorageManage.FrmOutSourcingMateReT.data1[1] = s2;
                    C23.StorageManage.FrmOutSourcingMateReT.data1[2] = s3;
                    C23.StorageManage.FrmOutSourcingMateReT.data1[3] = s4;
                    this.Close();
                }
                if (getdata == 2)
                {
                    ...
                    string s3 = this.dataGridView1.Rows[intCurrentRowNumber].Cells[2].Value.ToString().Trim();
                    string s4 = this.dataGridView1.Rows[intCurrentRowNumber].Cells[11].Value.ToString().Trim();
                    C23.StorageManage.FrmOutSourcingGodET.data4[0] = "doubleclick";
                    C23.StorageManage.FrmOutSourcingGodET.data1[0] = s1;
                    C23.StorageManage.FrmOutSourcingGodET.data1[1] = s2;
                    C23.StorageManage.FrmOutSourcingGodET.data1[2] = s3;
                    C23.StorageManage.FrmOutSourcingGodET.data1[3] = s4;
                    this.Close();
                }
            }
            else
            {
                FrmWorkOrderT frm = new FrmWorkOrderT();
                FrmWorkOrderT.str6[0] = Convert.ToString(dataGridView1[0, dataGridView1.CurrentCell.RowIndex].Value).Trim();
                frm.ShowDialog();
            }
        }
        public void a2()
        ...
        private void btnToExcel_Click(object sender, EventArgs e)
        {
            if (dt.Rows.Count > 0)
            {
                boperate.dgvtoExcel(dataGridView1, "工单明细");
            }
            else
            {
